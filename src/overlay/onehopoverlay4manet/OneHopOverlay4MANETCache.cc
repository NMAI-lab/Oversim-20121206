...
/**
 * @file OneHopOverlay4MANETCache.cc
 * @author Frank Ockenfeld
 * based on EpiChordFingerCache.cc, by Jamie Furness
 */
...
Define_Module(OneHopOverlay4MANETCache);
...
void OneHopOverlay4MANETCache::initialize(int stage)
{
...
void OneHopOverlay4MANETCache::handleMessage(cMessage* msg)
{
	throw new cRuntimeError("this module doesn't handle messages, it runs only in initialize()");
}

void OneHopOverlay4MANETCache::initializeCache(NodeHandle owner, OneHopOverlay4MANET* overlay, double ttl)
{
...
bool OneHopOverlay4MANETCache::contains(const TransportAddress& node)
{
...
void OneHopOverlay4MANETCache::updateEntry(const NodeHandle& node, bool direct, simtime_t lastUpdate, double ttl, NodeSource source)
{
	...
	OneHopOverlay4MANETCacheEntry entry;
...
void OneHopOverlay4MANETCache::setEntryTTL(const NodeHandle& node, double ttl)
{
	// Trying to update an unspecified node (happens if we receive a local findNode call)
...
bool OneHopOverlay4MANETCache::handleFailedNode(const TransportAddress& failed)
{
...
void OneHopOverlay4MANETCache::removeOldEntries()
{
	simtime_t now = simTime();
...
OneHopOverlay4MANETCacheEntry* OneHopOverlay4MANETCache::getNode(const NodeHandle& node)
{
	OverlayKey sum = node.getKey() - (thisNode.getKey() + OverlayKey::ONE);
...
OneHopOverlay4MANETCacheEntry* OneHopOverlay4MANETCache::getNode(uint32_t pos)
{
...
std::vector<OneHopOverlay4MANETCacheEntry> OneHopOverlay4MANETCache::getDeadRange(OverlayKey start, OverlayKey end)
{
	std::vector<OneHopOverlay4MANETCacheEntry> entries;
...
uint32_t OneHopOverlay4MANETCache::countSlice(OverlayKey start, OverlayKey end)
{
	uint32_t count = 0;
...
bool OneHopOverlay4MANETCache::isDead(const NodeHandle& node)
{
	OverlayKey sum = node.getKey() - (thisNode.getKey() + OverlayKey::ONE);
...
uint32_t OneHopOverlay4MANETCache::getSize()
{
	return liveCache.size();
}

uint32_t OneHopOverlay4MANETCache::countLive()
{
	return liveCache.size();
}

uint32_t OneHopOverlay4MANETCache::countRealLive()
{
	uint32_t count = 0;
...
uint32_t OneHopOverlay4MANETCache::countDead()
{
	return deadCache.size();
}

uint32_t OneHopOverlay4MANETCache::countRealDead()
{
	uint32_t count = 0;
...
void OneHopOverlay4MANETCache::findBestHops(OverlayKey key, NodeVector* nodes, std::vector<simtime_t>* lastUpdates, std::set<NodeHandle>* exclude, int numRedundantNodes)
{
...
simtime_t OneHopOverlay4MANETCache::estimateNodeLifetime(int minSampleSize)
{
	if (minSampleSize < 1)
		throw new cRuntimeError("OneHopOverlay4MANETCache::estimateNodeLifetime: minSampleSize must be > 0");
...
void OneHopOverlay4MANETCache::display()
{
	std::cout << "---------------------------" << std::endl;
