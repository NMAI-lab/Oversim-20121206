...
/**
 * @file OneHopOverlay4MANETIterativeLookup.cc
 * @author Frank Ockenfeld
 * based on EpiChordIterativeLookup.cc, by Jamie Furness
 */
...
OneHopOverlay4MANETIterativeLookup::OneHopOverlay4MANETIterativeLookup(BaseOverlay* overlay, RoutingType routingType, const IterativeLookupConfiguration& config, const cPacket* findNodeExt, bool appLookup) : IterativeLookup::IterativeLookup(overlay, routingType, config, findNodeExt, appLookup)
{
	onehopoverlay4manet = static_cast<OneHopOverlay4MANET*>(overlay);
}

IterativePathLookup* OneHopOverlay4MANETIterativeLookup::createPathLookup()
{
	return new OneHopOverlay4MANETIterativePathLookup(this, onehopoverlay4manet);
}

OneHopOverlay4MANETIterativePathLookup::OneHopOverlay4MANETIterativePathLookup(OneHopOverlay4MANETIterativeLookup* lookup, OneHopOverlay4MANET* onehopoverlay4manet) : IterativePathLookup(lookup)
{
	this->nextHops = LookupVector(lookup->config.redundantNodes * lookup->config.redundantNodes, lookup);
	this->onehopoverlay4manet = onehopoverlay4manet;
}

void OneHopOverlay4MANETIterativePathLookup::checkFalseNegative()
{
	...
	// There are dead nodes in between the 2 best options - alert their successor/predecessor
	if (!deadNodes->isEmpty())
		onehopoverlay4manet->sendFalseNegWarning(bestPredecessor, bestSuccessor, deadNodes);
...
void OneHopOverlay4MANETIterativePathLookup::handleResponse(FindNodeResponse* msg)
{
	if (finished)
		return;
	...
	if (!bestSuccessor.isUnspecified() && bestPredecessor.isUnspecified() && overlay->getThisNode().getKey().isBetween(bestSuccessor.getKey(), lookup->getKey())) {
		...
		bestPredecessorsSuccessor = onehopoverlay4manet->successorList->getNode();
	}
	...
	if (!bestPredecessor.isUnspecified() && bestSuccessor.isUnspecified() && overlay->getThisNode().getKey().isBetweenL(lookup->getKey(), bestPredecessor.getKey())) {
		...
		bestSuccessorsPredecessor = onehopoverlay4manet->predecessorList->getNode();
	}
...
void OneHopOverlay4MANETIterativePathLookup::handleTimeout(BaseCallMessage* msg, const TransportAddress& dest, int rpcId)
{
	if (finished)
		return;
...
LookupEntry* OneHopOverlay4MANETIterativePathLookup::getPreceedingEntry(bool incDead, bool incUsed)
{
...
LookupEntry* OneHopOverlay4MANETIterativePathLookup::getSucceedingEntry(bool incDead, bool incUsed)
{
...
LookupEntry* OneHopOverlay4MANETIterativePathLookup::getNextEntry()
{
	// First look for the closest node after the key
